...
public class Main {
    ...
    // Тест табулированного натурального логарифма
    ...
    public static void TestExter(){
        try {
            System.out.println("=== ТЕСТИРОВАНИЕ Externalizable ===");
    ...
    public static void testSerializable1(){
        try {
            System.out.println("=== ТЕСТ SERIALIZABLE ДЛЯ LinkedListTabulatedFunction ===");
            
            // Создаем функцию ln(exp(x)) = x
            Function composition = Functions.composition(new Log(Math.E), new Exp());
            
            // Создаем LinkedListTabulatedFunction
            LinkedListTabulatedFunction function = new LinkedListTabulatedFunction(0, 10, 11);
            double step = 10.0 / 10;
            for (int i = 0; i < 11; i++) {
                double x = i * step;
                function.setPointY(i, composition.getFunctionValue(x));
            }
            
            // Сериализация
            String filename = "linkedlist_serializable.ser";
            try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(filename))) {
                oos.writeObject(function);
            }
            
            // Десериализация
            LinkedListTabulatedFunction deserialized;
            try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(filename))) {
                deserialized = (LinkedListTabulatedFunction) ois.readObject();
            }

            // Вывод значений исходной и считанной функции
            System.out.println("\nСравнение значений функций на отрезке [0, 10] с шагом 1:");
            System.out.println("x\t\tИсходная\tСчитанная\tРазница");
            System.out.println("---------------------------------------------------");
